import {
  SEARCH_MANGA_AND_GENRES_QUERY,
  SEARCH_MANGA_QUERY,
  SEARCH_MANGA_SINGLE_QUERY,
} from "../queries/manga";
import type { Media, PageMedia, PartialMedia } from "$lib/types/manga";
...
export async function searchMangaAndGenres(params: SearchParams) {
  const res = await fetch("https://graphql.anilist.co", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      query: SEARCH_MANGA_AND_GENRES_QUERY,
      variables: {
        search: params.search || null,
        genre: params.genre !== "All" ? params.genre : null,
        sort: "POPULARITY_DESC",
  ...
  if (!res.ok) throw new Error("Failed to fetch manga and genre data");
  ...
  const data = (await res.json()).data as PageMedia;
  return data;
}
